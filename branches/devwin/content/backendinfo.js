...
function BackendInfo(filters, devmode) {
    ...
    this.cacheURL = function(url, callback) {
        ...
        // Cache URL now
        if (backendInfo.devmode) {
            if (url.indexOf(backendInfo.test_url) > -1) {
                backendInfo.devmode("Test: Random URL (always status 200)... status: ");
            } else {
                backendInfo.devmode("Cache: " + url + "... status: ");                    
            }   
        }
...
//                LOG("statusCode: " + httpRequest.status);
                if (backendInfo.devmode) {
                    backendInfo.devmode(httpRequest.status + "\n");      
                }
